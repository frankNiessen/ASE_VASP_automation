...
## Application on a High Performance Cluster (HPC)
The scripts can be run as-is on a personal computer,by default it will do so with *kpar = 1* and *ncore = 1*. If you have **VASP** and **ASE** installed on a high-performance cluster, or if you have a parallelized version of VASP compiled on your personal computer, you can supply the arguments *kpar* and *ncore* on the commnd line. As an example, you can run the calculations with *kpar = 2* and *ncore = 16* by executing: *job.py 2 16*
If you want to run *VASP* in a parallel environment, you will have to set the ASE environment *ASE_VASP_COMMAND* to *"mpirun vasp_std"*.
